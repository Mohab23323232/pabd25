...
def main():
    ...
    try:
        ...
        with open(os.path.join(results_dir, "metrics.json"), "w") as f_json:
            json.dump(metrics, f_json, indent=4)

        
        with open(os.path.join(results_dir, "report.txt"), "w", encoding="utf-8") as f_txt:
            f_txt.write("===== Evaluation Report =====\n")
            f_txt.write(f"MAE  : {metrics['mae']:.2f}\n")
            f_txt.write(f"RMSE : {metrics['rmse']:.2f}\n")
            f_txt.write(f"R²   : {metrics['r2']:.4f}\n\n")
            f_txt.write(f"Sample Prediction (60m², 10/25 floor, 2 rooms): {int(predicted_price)} руб\n")
